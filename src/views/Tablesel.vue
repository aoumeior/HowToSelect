<template>
  <div class="tablesel">
      ...
      <table>
        <tr>
            ...
            <td v-for="itemq in sy" :key="itemq">
              ...
              <select v-model="itemq.xx" v-if="itemq.kz==1" style="height:100px; width:100px" multiple @change="suoyin()">
                <option>全选</option>
      ...
      <span>第{{ page  }}页</span><span>/共{{ pages  }}页</span><span>每页{{  zpage }} 个</span>
      ...
      suoyin(){
        ...
        for(let i=0;i<this.yssj.length;i++){
          ...
          for(let p=0;p<this.sy.length;p++){
            if(this.kz[p]==1 && this.sy[p].xx!="全选"){
